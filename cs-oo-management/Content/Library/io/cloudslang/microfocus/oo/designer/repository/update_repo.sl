...
#! @output status_json: JSON document describing the SCM pull process status
...
flow:
  ...
  workflow:
    - trigger_update_repo:
        do:
          io.cloudslang.microfocus.oo.designer.repository.trigger_update_repo:
            - token: '${token}'
            - repo_id: '${repo_id}'
        publish:
          - process_json
    ...
    - sleep:
        do:
          io.cloudslang.base.utils.sleep:
            - seconds: "${get_sp('io.cloudslang.microfocus.oo.wait_time')}"
        navigate:
          - SUCCESS: get_process_status
          - FAILURE: on_failure
    - is_finished:
